...
def permute_matrix(mat, perm):
    """
    Permute the rows and columns of a matrix using a given permutation.
    """
...
class GraphDataset(torch.utils.data.Dataset):
    def __init__(self, matrices, targets, augment_permutation=True):
    ...
    def __getitem__(self, idx):
        ...
        # For 2x2, the only nontrivial permutation is swapping the two nodes.
        if self.augment_permutation and torch.rand(1).item() > 0.5:
            perm = [1, 0]
            x = permute_matrix(x, perm)
...
class GraphToGraphModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # Force symmetry: average the output with its transpose
        # out_sym = (out + out.transpose(-2, -1)) / 2
...
train_dataset = GraphDataset(train_mats, train_targets, augment_permutation=True)
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=4, shuffle=True)

# Initialize model and optimizer
model = GraphToGraphModel()
...
# --- Testing on 8x8 matrices ---
def generate_random_8x8_binary_matrix(num_samples):
    samples = []
    for _ in range(num_samples):
        mat = torch.randint(0, 2, (100, 100)).float()
        samples.append(mat)
    return samples


test_samples = generate_random_8x8_binary_matrix(8000)
