...
class DirectedGAEGIN(torch.nn.Module):
    def __init__(self, out_channels, hidden_channels, num_nodes):
        ...
        mlp = nn.Sequential(
            nn.Linear(hidden_channels, hidden_channels),  # First layer
            nn.ReLU(),  # Activation
            nn.Dropout(p=0.5),
            nn.Linear(hidden_channels, out_channels)  # Second layer to match output size
        )

        # Separate GCNs for incoming and outgoing edges
        self.gin_in = GINConv(mlp)
        self.gin_out = GINConv(mlp)
    ...
    def encode(self, edge_index_in, edge_index_out):
        ...
        z_in = self.gin_in(self.node_embeddings, edge_index_in)
        z_out = self.gin_out(self.node_embeddings, edge_index_out)
        z = z_in + z_out
        return z
