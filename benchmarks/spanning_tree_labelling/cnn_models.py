...
# -------------------------------------------
# Multi-objective score (unused here)
...
def train_policy(model, train_dl, val_dl, test_dl, dev,
    ...
    for e in range(1, epochs+1):
        ...
        print(f"Epoch {e:2d} ▶ val_acc {sum(vals)/len(vals):.4f}")
    # save best
...
if __name__=='__main__':
    ...
    model = CNNEdgeLabeler(NUM_NODES).to(dev)

    # 1) Supervised
    train_supervised(model, tr, va, te, dev, epochs=EPOCHS)

    # 2) Policy Soft
    train_policy(model, tr, va, te, dev,
