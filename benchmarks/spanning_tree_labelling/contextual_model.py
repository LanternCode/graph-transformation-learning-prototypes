...
class ContextAwareMLP(nn.Module):
    ...
    def forward(self, edge_features):  # edge_features: [E, 6]
        ...
        return torch.sigmoid(self.fc2(x)).squeeze(-1)  # output: [E]
...
def generate_candidate_graph(num_nodes):
    ...
    # Add extra edges
    all_possible = set(combinations(range(num_nodes), 2)) - original_edges
    extra_count = random.randint(1, min(len(all_possible), 2 * (num_nodes - 1)))
    extra_edges = random.sample(list(all_possible), extra_count)
...
def get_contextual_edge_features(G, edge_index):
    ...
    for u, v in edge_index:
        ...
        features.append([u, v, deg[u], deg[v], common, union])
...
def compute_loss(graph, scores, edge_index, lambda_weight=0.5):
    ...
    predicted_tree\
        = extract_tree_from_scores(edge_index, scores, num_nodes)
    predicted_labels = [(1 if (u, v) in predicted_tree or (v, u) in predicted_tree else 0)
                        for u, v in edge_index]
