...
class EdgeDisagreementBenchmark:
    """
    Benchmark runner for Facebook edge community-disagreement classification.

    Args:
        None.

    Returns:
        An object that can load the SNAP Facebook graph, extract balanced edge
        features and Louvain disagreement labels, and evaluate prediction
        adapters with a formatted classification report.
    """

    def __init__(self):
        """
        Initialise the benchmark by locating the dataset and loading the graph.
    ...
    def _ensure_dataset():
        """
        Ensure that the SNAP Facebook combined edge-list file is available.

        Args:
            None.

        Returns:
            None. The method downloads and extracts the dataset when the
            expected local edge-list file is missing.
        """
    ...
    def _load_graph(self):
        """
        Load the Facebook graph and assign Louvain community labels to nodes.

        Args:
            None.

        Returns:
            None. The loaded NetworkX graph is stored on ``self.graph`` with a
    ...
    def _extract_balanced_edge_data(self):
        """
        Build a balanced edge-level feature matrix and label vector.

        Args:
            None.

        Returns:
            A tuple ``(X, y)`` where ``X`` is a NumPy array of edge-level
            structural features and ``y`` is a NumPy array of binary labels
            indicating whether an edge connects two different communities.
        """
        features, labels = [], []
    ...
    def run(self, model_adapter_fn):
        """
        Evaluate a prediction adapter on the balanced edge-disagreement dataset.

        Args:
            model_adapter_fn: Callable that accepts a feature matrix ``X`` and
                returns a one-dimensional array-like sequence of predicted class
                labels.

        Returns:
            None. The method prints a formatted classification report.
        """
    ...
    def pretty_print_report(name, report_dict):
        """
        Print a compact tabular classification report.

        Args:
            name: Display name for the evaluated model or adapter.
            report_dict: Dictionary returned by ``classification_report`` with
                ``output_dict=True``.
