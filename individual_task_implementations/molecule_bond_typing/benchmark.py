...
def get_edge_examples(mol):
    ...
    Args:
        mol: RDKit molecule whose bonds are converted into benchmark examples.
...
def generate_benchmark_dataset(n=300):
    """
    Generate a benchmark dataset of bond examples from small molecule fragments.
    ...
    Returns:
        ...
        features and ``y`` is a NumPy array of remapped integer bond-class labels.
    """
    fragments = [
...
def run_bond_classification_benchmark(predict_fn, n=750):
    """
    Run the bond-type benchmark with a supplied prediction function.

    Args:
        predict_fn: Callable that accepts a NumPy feature matrix ``X`` and
            returns predicted integer bond-class labels.
        n: Number of fragment SMILES strings to sample for the benchmark set.

    Returns:
        None. The function prints prediction summaries and a scikit-learn
