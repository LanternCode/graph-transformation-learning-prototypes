...
def make_mlp_adapter(model_path, device='cpu', thresh=0.5):
    def adapter(graph):
        ...
        # 4) Return labels aligned with the benchmark edge_index order.
        return [preds[u * N + v] for u, v in graph['edge_index']]
...
def make_context_aware_adapter(model_path, device='cpu'):
    ...
    def adapter(graph):
        # Convert graph dict to directed NetworkX graph and preserve edge order.
        G = nx.DiGraph()
        ...
        with torch.no_grad():
            scores = torch.sigmoid(model(edge_features)).squeeze().cpu().numpy()
