...
def train_supervised(graph_train, graph_val, graph_test, device,
    ...
    for ep in range(1, epochs+1):
        ...
        if val_loss < best_val: best_val, best_state = val_loss, copy.deepcopy(model.state_dict())
...
def train_policy(graph_train, graph_val, graph_test, device,
    ...
    for ep in range(1, epochs+1):
        ...
        for g in graph_train:
            ...
            X = X.to(device)
            probs_all = torch.sigmoid(model(X))
            edge_positions = torch.tensor(
                [u * g['num_nodes'] + v for u, v in g['edge_index']],
                dtype=torch.long,
                device=device
            )
            probs = probs_all[edge_positions]
        ...
        if val_acc>best_val: best_val, best_state = val_acc, copy.deepcopy(model.state_dict())
