"""
Train and evaluate a CNN ablation without transpose access.

This prototype removes the transposed input channel used by the full symmetric
closure CNN. The model sees only A[i, j] at each position, so it tests whether a
shared local CNN rule can recover A OR Aᵀ without direct access to A[j, i].
"""
import torch
...
with torch.no_grad():
    for i, test_mat in enumerate(test_samples):
        ...
        pred = model(test_input).squeeze(0).squeeze(0)
