...
def generate_graph(num_nodes, p=None):
    ...
    Parameters:
        num_nodes (int): Number of nodes to include in the generated graph.
        p (float | None): Probability of adding each non-tree edge. When None,
            a value near the connectivity threshold, log(num_nodes) / num_nodes,
            is used.

    Returns:
        networkx.Graph: A connected undirected graph with num_nodes nodes.
    """
    if p is None:
...
def compute_edge_features_and_target(G):
    """
    Compute edge-level features and edge-betweenness targets for a graph.

    Parameters:
        G (networkx.Graph): Connected undirected graph whose edges are used as
            prediction examples.

    Returns:
        tuple[np.ndarray, np.ndarray, list[tuple[int, int]]]: A tuple containing
            X, y, and edge_list. X has shape (num_edges, 6) and contains
            [deg_u, deg_v, clust_u, clust_v, sum_deg, abs_diff_deg] for each
            edge. y has shape (num_edges,) and contains edge-betweenness
            centrality targets. edge_list contains the graph edges in the same
            order as X and y.
    """
...
def build_datasets(graphs):
    """
    Build flat-model arrays and GNN-ready graph tensors from NetworkX graphs.

    Parameters:
        graphs (list[networkx.Graph]): Graphs to convert into edge-level and
            graph-level training examples.

    Returns:
        tuple[np.ndarray, np.ndarray, list[dict[str, torch.Tensor]]]: A tuple
            containing X_all, y_all, and graph_data. X_all and y_all concatenate
            edge features and edge targets across all graphs for flat models.
            graph_data stores per-graph tensors needed by GCN and GraphSAGE.
    """
    X_list, y_list = [], []
    graph_data = []
...
class LinearModel(nn.Module):
    """
    Linear regression model for edge-betweenness prediction.

    Parameters:
        in_feats (int): Number of input edge features.

    Returns:
        LinearModel: A PyTorch module that maps edge features to one scalar
            prediction per edge.
    """

    def __init__(self, in_feats):
        """
        Initialize the linear edge-level regressor.

        Parameters:
            in_feats (int): Number of features in each edge-level input vector.
    ...
    def forward(self, x):
        """
        Predict edge-betweenness scores from edge-level features.

        Parameters:
            x (torch.Tensor): Tensor of shape (num_edges, in_feats) containing
                edge feature vectors.

        Returns:
            torch.Tensor: Tensor of shape (num_edges, 1) containing predicted
...
class MLP(nn.Module):
    """
    Multi-layer perceptron for edge-betweenness prediction.

    Parameters:
        in_feats (int): Number of input edge features.
        hidden (int): Hidden-layer width used by the MLP.

    Returns:
        MLP: A PyTorch module that maps edge features to one scalar prediction
            per edge.
    """

    def __init__(self, in_feats, hidden):
        """
        Initialize the MLP edge-level regressor.

        Parameters:
            in_feats (int): Number of features in each edge-level input vector.
            hidden (int): Number of hidden units in each hidden layer.
...
class GCN(nn.Module):
    """
    Graph convolutional network for edge-betweenness prediction.

    Parameters:
        in_feats (int): Number of input node features.
        hidden (int): Hidden-layer width used for node embeddings and edge MLP.

    Returns:
        GCN: A PyTorch module that embeds nodes with normalized adjacency and
            scores graph edges from endpoint embeddings.
    """
    ...
    def __init__(self, in_feats, hidden):
        ...
        Parameters:
            in_feats (int): Number of features in each node input vector.
            hidden (int): Number of hidden units in node and edge layers.
    ...
    def forward(self, node_feats, adj_norm, edge_index):
        """
        Predict edge-betweenness scores for edges in a graph.

        Parameters:
            node_feats (torch.Tensor): Tensor of shape (num_nodes, in_feats)
                containing node features.
            adj_norm (torch.Tensor): Normalized adjacency matrix with self-loops,
                shaped (num_nodes, num_nodes).
            edge_index (torch.Tensor): Tensor of shape (2, num_edges) containing
                source and destination node indices for each edge to score.

        Returns:
            torch.Tensor: Tensor of shape (num_edges,) containing predicted
                edge-betweenness scores.
        """
...
class GraphSAGE(nn.Module):
    """
    GraphSAGE-style model for edge-betweenness prediction.
    ...
    Returns:
        GraphSAGE: A PyTorch module that uses mean neighbor aggregation and
            scores graph edges from endpoint embeddings.
    """

    def __init__(self, in_feats, hidden):
        """
        Initialize the GraphSAGE-style model.
    ...
    def forward(self, node_feats, adj, degree, edge_index):
        ...
        Parameters:
            node_feats (torch.Tensor): Tensor of shape (num_nodes, in_feats)
                containing node features.
            adj (torch.Tensor): Raw adjacency matrix of shape
                (num_nodes, num_nodes), without self-loops.
            degree (torch.Tensor): Degree-like normalization vector of shape
                (num_nodes,) used for neighbor mean aggregation.
            edge_index (torch.Tensor): Tensor of shape (2, num_edges) containing
                source and destination node indices for each edge to score.
...
def main():
    """
    Train and evaluate the edge-betweenness prediction models.

    Parameters:
        None.

    Returns:
        None. The function prints training, validation, and test metrics, and
            writes the best model checkpoints to disk.
    """
    # Reproducibility
    seed = 42
    random.seed(seed)
    ...
    graphs = []
    for size in tqdm(sizes * num_per_size, desc="Graphs"):
