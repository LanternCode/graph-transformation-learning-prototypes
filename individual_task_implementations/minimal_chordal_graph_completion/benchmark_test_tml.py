...
def _shortest_paths(edge_index, num_nodes, pairs):
    """
    Compute shortest-path lengths for candidate pairs in a PyG graph.

    Args:
        edge_index: Tensor of graph edges used to rebuild an undirected NetworkX graph.
        num_nodes: Number of nodes in the graph.
        pairs: Tensor of candidate node pairs whose shortest paths are requested.

    Returns:
        Float tensor of shortest-path lengths, with -1 for disconnected pairs.
    """
...
def _rebuild_training_features(data):
    """
    Rebuild flat-model benchmark features in the original training order.

    Args:
        data: PyTorch Geometric Data object with edge_features, x, edge_pairs, and edge_index.

    Returns:
        A tensor of shape (num_candidate_edges, 8) in TRAIN_FEATURE_ORDER.
    """
...
def make_torch_adapter(model_ctor, weight_path,
                       ...
                       device=None):
    """
    Build a benchmark adapter for a trained PyTorch tabular fill-in model.

    Args:
        model_ctor: Model constructor accepting input_dim.
        weight_path: Path to the saved PyTorch state dictionary.
        scaler_path: Path to the fitted StandardScaler joblib file.
        feature_order_path: Path to the JSON feature-order file.
        device: Optional torch device. If omitted, CUDA is used when available.

    Returns:
        Callable that accepts one PyG Data object and returns fill-in probabilities.
    """
    if device is None:
    ...
    def adapter_fn(data):
        """
        Predict fill-in probabilities for one benchmark PyG graph.

        Args:
            data: PyTorch Geometric Data object containing candidate edge information.

        Returns:
            A one-dimensional probability tensor aligned with data.edge_pairs.
        """
...
def make_rf_adapter(pkl_path,
                    ...
                    feature_order_path="feature_order.json"):
    """
    Build a benchmark adapter for the trained Random Forest fill-in model.

    Args:
        pkl_path: Path to the saved Random Forest pickle/joblib file.
        scaler_path: Path to the fitted StandardScaler joblib file.
        feature_order_path: Path to the JSON feature-order file.
    ...
    def adapter_fn(data):
        """
        Predict Random Forest fill-in probabilities for one benchmark PyG graph.
